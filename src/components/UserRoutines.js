...
const UserRoutines = ({
  ...
  return (
    <div className="d-flex flex-column align-content-center">
      ...
      <button
        className="mt-3 ml-3 align-self-start btn btn-outline-light mt-3"
        onClick={() => {
          setCreateRoutineActive(!createRoutineActive);
        }}
      >
        {!createRoutineActive ? "Create New Routine" : "Hide New Routine Form"}
      </button>
      {createRoutineActive && (
        <CreateRoutineForm token={token} setUserRoutines={setUserRoutines} />
      )}
